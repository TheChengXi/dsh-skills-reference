 ...
 * 到 ctx.typert（使浏览器可调 list/replace 读写声明），effect 卸载时回收内层实例与声明 watcher。
 *
...
import { readReferences, resolveReferencesFile } from "./references-store.js";
...
function watchReferencesFile(cwd: string, onChange: () => void): () => void {
  const { filePath } = resolveReferencesFile(cwd);
  const listener = () => onChange();
  watchFile(filePath, { persistent: false, interval: 500 }, listener);
